...
var numRabbits = function (answers) {
	...
	// 将每个数量的兔子进行分组
	// 如果一只兔子说有 key 数量的兔子与它颜色相同，则最少有 key + 1只兔子
	// 例如，一只兔子说有 2只 兔子颜色与它相同，则最少有3只兔子
	// 但是 另外两只兔子 也可能出现在数组中，所以根据贪心原则，
	// 如果出现了 说 2只颜色与它相同 的兔子，就当作是 另外两只兔子 中的，这样就能让兔子总数量最小

	// 所以，让 value / (key + 1) 就可以得到 说某种颜色的兔子 总共有多少组，不足1组的兔子 额外算做1组。
	// 例如，有 8只 兔子说有 “2只兔子颜色与它相同” ，8 / (2+1) , 得 2 余 2，说明有2组兔子，另外剩下的两只兔子额外成1组。
	// 在计算中就只要向上取整，有余数的情况就向上多取一组
	// 每组的数量最少有 key+1 只，最后乘以 key+1 ，就是说某个数量的兔子最少有多少只。
  let ans = 0;
